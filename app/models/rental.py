...
class Rental(TimestampMixin, Base):
    ...
    ``start_date`` / ``end_date`` are the agreed term, both set when the rental is
    registered. ``returned_at`` is ``None`` while the car is still out and is set
    when the rental is ended; an *active* rental is one with ``returned_at IS NULL``.
    """
